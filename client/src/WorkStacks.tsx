...
export type WorkStacksProps={folders:string[];images:Project[];stacks?:StacksSettings};
type Stack={name:string;items:Project[];cover:Project};

/** The scatter-to-organize entrance plays once per page load, however often the panel re-renders or remounts. */
...
/* ============================================================ organized stacks ===== */
function StackGrid({stacks,reduced,onOpen}:{stacks:Stack[];reduced:boolean;onOpen:(name:string,opener:HTMLElement)=>void}){
  const rootRef=useRef<HTMLDivElement>(null);
  const [entered,setEntered]=useState(reduced||entrancePlayed);
  ...
  useEffect(()=>{
    ...
    function play(){
      if(started||entrancePlayed)return;started=true;entrancePlayed=true;
...
/* ============================================================ panel ===== */
export function WorkStacks({folders,images,stacks:settings}:WorkStacksProps){
  ...
  return <section className="work-stacks" aria-label="Work">
    {current
      ...
      :<StackGrid stacks={stacks} reduced={reduced} onOpen={(name,el)=>{opener.current=el;setOpen(name);}}/>}
  </section>;
}
