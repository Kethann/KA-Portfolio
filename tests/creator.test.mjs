...
test('private creator workflow, persistent portfolio/images/inbox, and public isolation',async()=>{
  ...
  try{
    ...
    assert.equal((await request('/api/creator/session',{headers:{Origin:'https://another.example','X-Forwarded-Host':'another.example'}})).status,403,'untrusted forwarded host cannot bypass origin checks');
    ...
    const logoUpload=await request('/api/creator/upload',{method:'POST',headers:{'Content-Type':'image/png'},body:png});
    const logo=(await logoUpload.json()).src;publicSite.branding={enabled:true,logoUrl:logo};
    ...
    const library=await (await request('/api/creator/assets')).json();assert.equal(library.find(asset=>asset.src===logo).used,true);
    assert.equal((await request('/api/creator/assets/'+logo.split('/').pop(),{method:'DELETE'})).status,409,'studio logo remains protected');
    ...
    const {readdir}=await import('node:fs/promises');const archived=await readdir(resolve(dataDir,'archived-uploads'));
    assert(archived.some(name=>name.endsWith(src.split('/').pop())),'removed image is recoverable');
