...
function harness(){
  const listeners=new Map(),media=new Map(),frames=new Map();let now=1,id=0,strokes=0;
  const listen=(type,fn)=>{const list=listeners.get(type)||[];list.push(fn);listeners.set(type,list);};
  const context2d=new Proxy({stroke(){strokes++;},createLinearGradient:()=>({addColorStop(){}}),createRadialGradient:()=>({addColorStop(){}})},{get:(target,key)=>target[key]||(()=>{})});
  ...
  return {context,canvas,document,media,frames,get strokes(){return strokes;},emit(type,event={}){for(const fn of listeners.get(type)||[])fn(event);},step(ms=17){now+=ms;const batch=[...frames.values()];frames.clear();batch.forEach(fn=>fn(now));}};
}
...
test('background bounds pixel cost, throttles rendering, and cancels hidden/reduced-motion frames',()=>{
  ...
  for(let i=0;i<3600;i++)h.step(100);
  let field;renderer.scene.traverse(o=>{if(o.isPoints&&o.geometry.getAttribute('aTwinkle'))field=o;});
  assert(field,'background star field exists');
  assert(Math.abs(field.rotation.x)<=.025&&Math.abs(field.rotation.y)<=.045,'six minutes of drift cannot rotate stars out of view');
  assert(field.material.vertexShader.includes('0.85 + 0.15'),'random twinkle retains a visible baseline');
...
test('ambient glow particles remain visible through repeated lifetimes without cursor stars',()=>{
  const h=harness();h.context.window.matchMedia('(any-pointer: fine)').matches=true;
  let source=scripts.find(s=>s.includes('function trackTrailPointer'));
  assert(!source.includes('starSprite'),'cursor star addition is removed');
  const end=source.lastIndexOf('})();');
  source=source.slice(0,end)+'globalThis.ambientTest=embers;'+source.slice(end);
  vm.runInContext(source,h.context);
  assert.equal(h.context.ambientTest.length,12);
  for(let i=0;i<3600;i++){
    h.step(100);
    for(const e of h.context.ambientTest){
      assert(e.y>=0&&e.y<=844,'glow remains within the screen');
      assert(Number(e.el.style.opacity)>=.299,'glow never expires to invisible');
    }
  }
  h.document.hidden=true;h.emit('visibilitychange');assert.equal(h.frames.size,0);
  h.document.hidden=false;h.emit('visibilitychange');h.step();
  assert(h.context.ambientTest.every(e=>Number(e.el.style.opacity)>=.299));
});
