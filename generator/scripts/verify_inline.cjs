...
for(const [index,match] of [...html.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/g)].entries()){
  const results=new Linter().verify(match[2],[{languageOptions:{ecmaVersion:2022,sourceType:match[1].includes('module')?'module':'script',globals:{...globals.browser,THREE:'readonly',kaApiUrl:'readonly',kaLayoutW:'readonly',kaLayoutH:'readonly'}},rules:{'no-undef':'error'}}]);
